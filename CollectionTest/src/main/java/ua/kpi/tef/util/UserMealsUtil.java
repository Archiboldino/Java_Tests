...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesForDays = mealList.stream()
                .collect(Collectors.groupingBy(m -> LocalDate.from(m.getDateTime()),
                        Collectors.summingInt(UserMeal::getCalories)));

        return mealList.stream()
                .filter(m -> TimeUtil.isBetween(LocalTime.from(m.getDateTime()), startTime, endTime))
                .map(m -> new UserMealWithExceed(m.getDateTime(), m.getDescription(), m.getCalories(),
                        caloriesForDays.get(LocalDate.from(m.getDateTime())) > caloriesPerDay)).collect(Collectors.toList());
    }
}
